...
<html>
   <head>
      ...
      function handleCheckData(){
         ...
         var uidResult = uidPattern.test($("#uid").value);
         var el_uid_span = $("#uidSpan");
         if(uidResult) {
        	 $("#uidSpan").classList.addClass("text-danger");

         } else {
        	 $("#uidSpan").classList.remove("text-danger");
         ...
         if(emailResult) {
        	 $("#email").remove("bg-danger");
         } else {
        	 $("#email").classList.addClass("bg-danger");
         }
         ...
         if(phoneResult) {
        	 ...
        	 $("#phoneSpan").classList.add("text-danger");
         }
         
         //5)전체 효율성 검사 결과가 true일 경우
         if(totalResult) {
        	 var el_form = $("#joinForm");
        	 // 강제적으로 action 기능을 수행하도록 함
        	 $("#joinForm")[0].submit(); // [0] 인덱스 엘리먼트 <form>
         }
      }
   </script>
